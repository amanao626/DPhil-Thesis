...
avg_dataN <- Ndata %>%
  group_by(L1, Pro) %>%
  summarise(
    avg_res2 = mean(res, na.rm = TRUE),           # Mean for each group
    n        = n(),                               # Sample size for each group
    se       = sd(res, na.rm = TRUE) / sqrt(n),   # Standard error
    ci_lower = avg_res2 - qt(0.975, df = n - 1) * se,  # 95% CI (lower)
    ci_upper = avg_res2 + qt(0.975, df = n - 1) * se,  # 95% CI (upper)
    .groups  = "drop"
  )
...
# 3.6 Display on screen (RGui)
windows() 
print(N)

# 4. Notes on fonts (if needed)
