...
# 2. Load data
Ndata <- read_csv("Nonsense sentences.csv")
...
res_sub <- recalculateResiduals(res_glmm, group = Ndata$sub)
...
res_item <- recalculateResiduals(res_glmm, group = Ndata$item)
...
# 8. Part R² and Cohen's f² for Pro (model4 vs. model6)  [*Note: model5 removes Pro; model6 removes L1*]
...
Ndata$Japanese_z <- (Ndata$Japanese - mean(Ndata$Japanese, na.rm = TRUE)) /
  sd(Ndata$Japanese, na.rm = TRUE)

# 11.2 Working memory (center + standardize, SD = 1)
Ndata$WM_z <- (Ndata$WM - mean(Ndata$WM, na.rm = TRUE)) /
  sd(Ndata$WM, na.rm = TRUE)

# 11.3 Number of sounds: treat as categorical (factor)
Ndata$NSo <- as.factor(Ndata$NS)
