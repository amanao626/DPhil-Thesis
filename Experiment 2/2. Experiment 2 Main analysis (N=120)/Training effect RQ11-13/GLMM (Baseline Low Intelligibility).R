...
# 2. Load data
LIdata <- read_csv("InteLowAllEx2.csv")

# 3. Recode variables
# 3.1 Effect coding (mean-centering) for binary predictors
#     Note: scale(x, scale = FALSE) subtracts the mean (i.e., centers x).
LIdata$Accent  <- scale(LIdata$Accent,  scale = FALSE)
LIdata$Caption <- scale(LIdata$Caption, scale = FALSE)
LIdata$L1      <- scale(LIdata$L1,      scale = FALSE)
head(LIdata)
...
#     Model 3 (retain Accent:Caption; drop Accent:L1 and keep main-effect slopes)
model3 <- glmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
...
#     Model 4 (retain Accent:Caption; drop Caption slope)
model4 <- glmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
...
#     Model 5 (retain Accent:Caption; drop Accent slope)
model5 <- glmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
...
res_sub <- recalculateResiduals(res_glmm, group = LIdata$sub)
plot(res_sub)
...
res_item <- recalculateResiduals(res_glmm, group = LIdata$item)
plot(res_item)
