...
# 2. Load data
LCdata <- read_csv("CompLowAllEx2.csv")

# 3. Recode variables
# 3.1 Effect coding (mean-centering) for binary predictors
#     Note: scale(x, scale = FALSE) subtracts the mean (i.e., centers x).
LCdata$Accent  <- scale(LCdata$Accent,  scale = FALSE)
LCdata$Caption <- scale(LCdata$Caption, scale = FALSE)
LCdata$L1      <- scale(LCdata$L1,      scale = FALSE)
head(LCdata)
...
model3 <- lmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
    (1 | sub) + (1 + Accent:Caption + L1 + Accent + Caption || item),
  data = LCdata,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000))
)
summary(model3)
...
model4 <- lmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
    (1 | sub) + (1 + Accent:Caption + L1 + Accent || item),
  data = LCdata,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000))
)
summary(model4)
...
model5 <- lmer(
  res ~ Accent*Caption + Accent*L1 + PreScorez +
    (1 | sub) + (1 + Accent:Caption + L1 || item),
  data = LCdata,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000))
)
summary(model5)
...
anova(model5, model7, refit = FALSE)

# 5. Model diagnostics with DHARMa (based on model5)
# 5.1 Simulate residuals
res_lmm <- simulateResiduals(model5, plot = FALSE)
...
# 6. R² with performance (based on model5)
r2_vals <- r2(model5)
print(r2_vals)

# 7. Part R² and Cohen's f² for each fixed effect (based on model5)
r2_full <- r2_nakagawa(model5)$R2_marginal
cat("R2_marginal (full) =", r2_full, "\n")

# 7.1 Drop Accent
red_accent <- update(model5, . ~ . - Accent)
...
# 7.2 Drop Caption
red_caption <- update(model5, . ~ . - Caption)
...
# 7.4 Drop PreScorez
red_pre <- update(model5, . ~ . - PreScorez)
...
red_accent_caption <- update(model5, . ~ . - Accent:Caption)
...
red_accent_L1 <- update(model5, . ~ . - Accent:L1)
